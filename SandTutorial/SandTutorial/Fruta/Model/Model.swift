...
class Model: ObservableObject {
    ...
    private var userCredential: String? {
        get { defaults?.string(forKey: "UserCrendential") }
        set { defaults?.setValue(newValue, forKey: "UserCrendential") }
    }
    ...
    init() {
        ...
        // refunds the purchase or if a parent approves a child's request to buy.
        updatesHandler = Task {
...
extension Model {
    ...
    var searchSuggestions: [Ingredient] {
        Ingredient.all.filter {
            $0.name.localizedCaseInsensitiveContains(searchString) && $0.name.localizedCaseInsensitiveCompare(searchString) != .orderedSame
        }
    }
    ...
    func purchase(product: Product) {
        Task { @MainActor in
            do {
                ...
                guard case .success(.verified(let transaction)) = result, transaction.productID == Model.unlockAllRecipesIdentifier else {
                    return
                }
                self.allRecipesUnlocked = true
            } catch {
                print("Failed to purhcase \(product.id): \(error)")
            }
        }
    }
...
extension Model {
    ...
    private func fetchProducts() {
        Task { @MainActor in
            ...
            self.unlockAllRecipesProduct = self.fetchedProducts.first { $0.id == Model.unlockAllRecipesIdentifier }
    ...
    private func updateAllRecipesOwned() async {
        ...
        guard let entitlement = await product.currentEntitlement, case .verified(_) = entitlement else {
            self.allRecipesUnlocked = false
            return
        }
        self.allRecipesUnlocked = true
    ...
    /// - Important: This method never retunrs, it will only suspend.
    @MainActor
