        ...
        if var cell = cell {
            ...
            if active {
                cell.createNewParticles(delta: delta) {
                    Particle(position: position, parentCenter: position, velocity: velocity, size: size, gradientIndex: gradientIndex)
                }
            }
            active = active || cell.isActive
            self.cell = cell
        } else {
            if (opacitySpeed <= 0 && opacity <= 0) || (sizeSpeed <= 0 && size <= 0) {
                active = false
            }
        }
        return active
