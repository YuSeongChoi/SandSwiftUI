...
struct HighAnimationView_Previews: PreviewProvider {
...
// MARK: - Example 3: Polygon with multiple animation parameters
...
// MARK: - Example 4: Polygon with line vertex-to-vertex
...
struct ClockTime {
    ...
    // Initializer with hour, minute and seoncds
    ...
    // Initializer with titak if seconds
...
struct PetalShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let sep = arc / 2
        let to = CGPoint(x: CGFloat(cos(angle.radians) * Double(hypotenuse)) + center.x, y: CGFloat(sin(angle.radians) * Double(hypotenuse)) + center.y)
        
        let ctrl1 = CGPoint(x: CGFloat(cos((angle + sep).radians) * Double(hypotenuse)) + center.x, y: CGFloat(sin((angle + sep).radians) * Double(hypotenuse)) + center.y)
        
        let ctrl2 = CGPoint(x: CGFloat(cos((angle - sep).radians * Double(hypotenuse)) + center.x), y: CGFloat(sin((angle - sep).radians) * Double(hypotenuse)) + center.y)
    ...
    var body: some View {
        GeometryReader { proxy in
            ZStack(alignment: .topLeading) {
                ...
                Image(systemName: "airplane").resizable().foregroundColor(.red)
...
struct PercentageIndicator: AnimatableModifier {
    ...
    struct ArcShape: Shape {
        ...
        func path(in rect: CGRect) -> Path {
            var p = Path()
            p.addArc(center: CGPoint(x: rect.width / 2.0, y: rect.height / 2.0), radius: rect.height /  2.0 + 5.0, startAngle: .degrees(0), endAngle: .degrees(360.0 * Double(pct)), clockwise: false)
    ...
    var body: some View {
        ...
        return VStack {
            ...
            Color.clear.frame(width: 200, height: 200)
                .overlay(Color.clear
                    .modifier(AnimatableGradient(from: gradient1, to: gradient2, pct: animate ? 1 : 0)))
            Spacer()
...
struct AnimatableGradient: AnimatableModifier {
    ...
    // This is a very basic implementation of a color interpoliation between two values.
...
struct WaveText: View {
    ...
    var body: some View {
        Text(text).foregroundColor(Color.clear)
            .modifier(WaveTextModifier(text: text, waveWidth: waveWidth, pct: pct, size: size))
    }
