    ...
    func body(content: Content) -> some View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack(alignment: .top, spacing: 0) {
                    Image(item.image)
                        ...
                        .overlay(
                            StarView(isFavorite: isFavorite && flyingFromGrid)
                                .opacity(Double(1-pct)), alignment: .topTrailing
                        )
                        .clipShape(RectangleToCircle(cornerRadiusPercent: cornerRadius))
                        .overlay(
                            RectangleToCircle(cornerRadiusPercent: cornerRadius)
                                .strokeBorder(Color.white, lineWidth: 2 * borderPct)
                        )
                        .padding(2 * borderPct)
                        .overlay(
                            RectangleToCircle(cornerRadiusPercent: cornerRadius)
                                .strokeBorder(Color.black.opacity(0.1 * Double(borderPct)))
                        )
        ...
        .overlay(
            CloseButton(onTap: onClose)
                .opacity(Double(pct)), alignment: .topTrailing
        )
