...
/// if the modal is presented, by doubl-tapping on it.
...
struct ConfigPopover: View {
    ...
    var body: some View {
        let slowAnimations = Binding<Bool> {
            _slowAnimations
        } set: {
            _slowAnimations = $0
            debugAnimations = $0
        ...
        return Form {
            Section(header: Text("Thumbnail Configuration")) {
                ...
                VStack(alignment: .leading, spacing: vStackSpacing) {
                    HStack {
                        Spacer()
                        Text("Width \(Int(config.thumbnailSize.width))")
                        Spacer()
                    ...
                    HStack {
                        ...
                        Slider(value: $config.thumbnailSize.height, in: 50...136)
                    }
                }.padding(.vertical, vStackPadding)
            ...
            Section(header: Text("Other")) {
                Toggle("Dark Model", isOn: self.$config.darkMode)
