...
struct Wildlife: View {
    ...
    // Determine if geometry matched occur
    ...
    var body: some View {
        ZStack {
            ...
            if animalTap != nil || favoriteTap != nil {
                ModalView2(id: animalTap ?? favoriteTap!,
    ...
    func openModal(_ item: ItemData2, fromGrid: Bool) {
        if fromGrid {
            animalTap = item.id
        } else {
            favoriteTap = item.id
    ...
    func toggleFavoriteStatus(_ item: ItemData2) {
