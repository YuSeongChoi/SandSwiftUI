...
class BoardClass: ObservableObject {
    ...
    func makeNewBoard() {
        ...
        for (i, ch) in currentPuzzle[0].map(String.init).enumerated() {
            items.append(Square(value: (Int(ch) ?? 0), index: i, rowCol: indexToRowCol(index: i), isPermanent: (Int(ch) ?? 0) > 0))
        }
    }
    ...
    func isValid(rowIndex: Int, colIndex: Int, k: Int) -> Bool {
        for i in 0..<9 {
            ...
            if (items[rowColToIndex(row: rowIndex, col: i)].squareVal == k || items[rowColToIndex(row: i, col: colIndex)].squareVal == k || items[rowColToIndex(row: m, col: n)].squareVal == k) {
                return false
            }
        }
        return true
